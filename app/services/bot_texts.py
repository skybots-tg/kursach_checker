...
SYSTEM_TEXTS: dict[str, dict[str, Any]] = {
    "bot.welcome": {
        ...
        "default": (
            "<b>Кидай файл</b> 👇🏼\n"
            "проверю и помогу привести оформление по основным ГОСТам:\n"
            "ГОСТ 2.105-95, ГОСТ 7.32-2001, ГОСТ Р 6.30-2003 и другим базовым стандартам, "
            "на которых строятся методические рекомендации вузов\n"
            "\n"
            "у тебя есть <b>1 бесплатная попытка</b>\n"
            "дальше каждая проверка — <b>150 ₽</b>💸\n"
            "\n"
            "поля, интервалы, нумерация, структура, библиография — "
            "всё, на чем чаще всего ловят перед сдачей\n"
            "\n"
            "<b>загружай файл, посмотрим, что можно исправить</b> 🔥"
        ),
        "supports_html": True,
        "variables": [],
    },
    "bot.how_it_works": {
        "label": "Как работает бот",
        "group": "Общие",
        "description": "Объяснение работы бота, может использоваться в меню/follow-up",
        "default": (
            "<b>как работает бот? всё просто:</b>\n"
            "\n"
            "1️⃣ загружаешь файл\n"
            "2️⃣ бот проверяет и помогает с оформлением\n"
            "3️⃣ смотришь результат и решаешь, что делать дальше\n"
            "\n"
            "🎁 у тебя есть <b>1 бесплатная проверка</b> — можешь спокойно протестить)\n"
            "\n"
            "p.s. кстати, за подписку на наш тг-канал ты получишь <b>+ {subscribe_bonus} бесплатные проверки</b> "
            "(ссылка в кнопке «Польза для студентов»)\n"
            "\n"
            "а если приглашаешь друга, <b>+1</b> — за каждого 🔥"
        ),
        "supports_html": True,
    ...
    "bot.kb_anchor": {
        ...
        "group": "Общие",
        "description": (
            "Короткое сообщение, которое бот отправляет, чтобы reply-клавиатура "
            "(нижнее меню) всегда оставалась видимой. В Telegram reply-клавиатура "
            "«живёт» вместе с сообщением, к которому прикреплена, и пропадает при "
            "его удалении. Якорь шлётся только тогда, когда привязать клавиатуру "
            "к одному из контентных сообщений (без inline-кнопок) нельзя."
        ),
        "default": "▾",
        "supports_html": True,
        "variables": [],
    ...
    "check.no_credits": {
        ...
        "default": (
            "<b>Бесплатная проверка использована 🥺</b>\n"
            "\n"
            "дальше можно идти без паузы: <b>по 150₽ за оформленную работу</b>.\n"
            "\n"
