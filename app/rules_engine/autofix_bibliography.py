...
_BIBLIOGRAPHY_PATTERN = re.compile(
    r"^\s*(?:"
    r"список(?:\s+(?:использованн(?:ых|ой)|используем(?:ых|ой)))?\s+(?:источник(?:ов|и)(?:\s+и\s+литератур[ыа])?|литератур[ыа](?:\s+и\s+источник(?:ов|и))?)"
...
_NPA_PATTERNS = re.compile(
    ...
    r"|приказ\s+(?:мин|росстат|фас|фнс|фтс|федеральн)"
    r"|кодекс"
    r"|№\s*\d+[\-\u2013\u2014]?(?:фз|фкз)"
    r"|гост\s+р?\s*\d"
    r"|договор\s+о\b"
    r"|таможенн\w+\s+кодекс"
    r"|об\s+утверждении\b"
    r"|трудов\w+\s+кодекс"
    r"|гражданск\w+\s+кодекс"
    r"|налогов\w+\s+кодекс"
    r"|бюджетн\w+\s+кодекс"
    r")",
    re.IGNORECASE | re.UNICODE,
)
