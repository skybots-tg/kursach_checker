...
import { fetchDemoCheck } from "../api";
...
export const DemoPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true);
        const res = await fetchDemoCheck();
        if (!cancelled) {
          setData(res);
        }
      } catch (e) {
        if (!cancelled) {
          setError(e instanceof Error ? e.message : "Ошибка загрузки демо");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="page-section-description">
            Ниже пример реального отчёта по небольшой работе: ошибки, предупреждения и подсказки.
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="secondary-btn" style={{ flex: 1 }} disabled>
              <Icon name="download" className="bottom-nav-icon" /> Скачать пример работы
            </button>
          </div>
        </>
      )}
